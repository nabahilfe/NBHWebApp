...
Dcumentation of the data that is generated or requred for this project 
...
Set yor password by registering wit the email address above. 
This user has all permissions and is used for administration of the system. 

This initail Setup generates also the required Administrator role and assigns it to the user.

Both entitis, the user and the role, are not editable nor deletable.
...
- All incoming mails are silentliy discarded, so please do not send any emails to this address.
...
- all incoming emails are deleted after forwaring to the configured accounts.
